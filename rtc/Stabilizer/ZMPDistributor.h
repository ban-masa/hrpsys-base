...
//             ref_foot_moment[1] = foot_dist_coords_rot * ref_foot_moment[1];
        ...
        const hrp::Vector3& rope_dir, const hrp::Vector3& hand_zmp, const size_t grasping_hand_num)
    {
      ...
      //For Feet CoP
      {
        ...
        for (size_t i = 0; i < is_contact_list.size(); i++) {
          if (is_contact_list[i]) {
            hrp::dmatrix lcmat = ee_local_contact_matrix_list[i].block(3, 0, 3, ee_local_contact_matrix_list[i].cols());
            Phimat.block(6 + 3 * ee_idx, col_idx, 3, ee_local_contact_matrix_list[i].cols()) = lcmat;
            ee_idx++;
            col_idx += ee_local_contact_matrix_list[i].cols();
          }
        }
      }
        ...
        )
    {
      ...
      if (grasping_hand_num == 0) {
        ...
        A.block(0, 3, 3, 3) = hrp::dmatrix::Identity(3, 3);
        A.block(3, 0, 3, 3) = cross_mat1;
        A.block(3, 3, 3, 3) = cross_mat2;
        hrp::calcPseudoInverse(A, A_inv);
        hrp::dvector ret = hrp::dvector::Zero(6);
        hrp::dvector b = hrp::dvector::Zero(6);
        for (size_t i = 0; i < 3; i++) b[i] = rope_force[i];
        ret = A_inv * b;
        ...
        const bool printp = true, const std::string& print_str = "")
    {
      ...
      for (size_t i = 0; i < ee_num; i++) {
        if (is_contact_list[i]) {
          ...
          tmpv = ee_matrix_list[i] * temp_state;
          ref_foot_force[i] = hrp::Vector3(tmpv(0), tmpv(1), tmpv(2));
          ref_foot_moment[i] = hrp::Vector3(tmpv(3), tmpv(4), tmpv(5)) - ee_pos[i].cross(ref_foot_force[i]);
        } else {
          ref_foot_force[i] = hrp::Vector3::Zero();
      ...
      if (grasping_hand_num > 0) {
        double rope_tension = rho_vec(state_dim - 1);
      ...
      if (printp) {
        ...
        std::cerr << "hand zmp: " << hand_zmp(0) << " " << hand_zmp(1) << " " << hand_zmp(2) << std::endl;
        std::cerr << "rope dir: " << rope_dir(0) << " " << rope_dir(1) << " " << rope_dir(2) << std::endl;
        for (size_t i = 0; i < ee_num; i++) {
        ...
        for (size_t i = 0; i < hand_contact_list.size(); i++) {
          ...
          std::cerr << "act hand moment: " << act_hand_moment[i](0) << " " << act_hand_moment[i](1) << " " << act_hand_moment[i](2) << std::endl;
        }
        if (grasping_hand_num > 0) {
